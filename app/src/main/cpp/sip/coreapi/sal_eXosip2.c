...
void sal_get_default_local_ip(Sal *sal, int address_family, char *ip, size_t iplen) {
	if (eXosip_guess_localip(sal->osipCtx, address_family, ip, iplen) < 0) {
...
void sal_op_release(SalOp *op) {
	...
	if (op->rid != -1) {
		...
		sal_remove_register(sal, op->rid);
		eXosip_register_remove(sal->osipCtx, op->rid);
	}
	if (op->cid != -1) {
...
Sal * sal_init() {
	...
	sal->add_dates = FALSE;
	sal->dscp = -1;
	sal->transport = SalTransportUDP;

	sal->osipCtx = eXosip_malloc();
	eXosip_init(sal->osipCtx);

	int val = 1;
	eXosip_set_option(sal->osipCtx, EXOSIP_OPT_AUTO_MASQUERADE_CONTACT, &val);
	return sal;
}

void sal_uninit(Sal* sal) {
	eXosip_quit(sal->osipCtx);
	osip_free(sal->osipCtx);
	sal->osipCtx = NULL;
...
int sal_unlisten_ports(Sal *ctx) {
	if (ctx->running) {
		eXosip_quit(ctx->osipCtx);
		eXosip_init(ctx->osipCtx);
		ctx->running = FALSE;
	}
	return 0;
}
...
int sal_reset_transports(Sal *ctx) {
	...
	if (ctx->running) {
		...
		eXosip_reset_transports(ctx->osipCtx);
	}
	return 0;
#else
...
static void set_tls_options(Sal *ctx) {
	...
	eXosip_tls_verify_certificate(ctx->osipCtx, ctx->verify_server_certs);
#endif
}
...
void sal_set_dscp(Sal *ctx, int dscp) {
	...
	if (dscp != -1)
		eXosip_set_option(ctx->osipCtx, EXOSIP_OPT_SET_DSCP, &ctx->dscp);
#endif
}

int sal_get_dscp(Sal *ctx) {
	return ctx->dscp;
}
...
int sal_listen_port(Sal *ctx, const char *addr, int port, SalTransport tr, int is_secure) {
	...
	eXosip_set_option(ctx->osipCtx, EXOSIP_OPT_USE_RPORT, &use_rports);
	...
	eXosip_set_option(ctx->osipCtx, EXOSIP_OPT_ENABLE_IPV6, &ipv6);

	//if (is_secure && tr == SalTransportUDP) {
	//	ms_fatal("SIP over DTLS is not supported yet.");
	//	return -1;
	//}
	err = eXosip_listen_addr(ctx->osipCtx, proto, addr, port, ipv6 ? PF_INET6 : PF_INET, is_secure);
	ctx->running = TRUE;
	return err;
}

void sal_set_user_agent(Sal *ctx, const char *user_agent) {
	eXosip_set_user_agent(ctx->osipCtx, user_agent);
}
...
void sal_use_dates(Sal *ctx, bool_t enabled) {
	...
	{
		int tmp = enabled;
		eXosip_set_option(ctx->osipCtx, EXOSIP_OPT_REGISTER_WITH_DATE, &tmp);
	}
#else
...
void sal_add_auth_info(Sal *sal, const SalAuthInfo *info)
{
	const char *userid;
	...
	eXosip_add_authentication_info(sal->osipCtx, info->username, userid,
...
#ifdef HAVE_EXOSIP_TLS_VERIFY_CERTIFICATE
	eXosip_tls_verify_certificate(ctx->osipCtx, verify);
#endif
}
...
int sal_call_set_local_sdp(SalOp *h, const char* sdp)
{
	if (sdp == NULL)
	{
		return -1;
	...
	h->base.local_sdp = sdp;

	return 0;
}

int sal_call_set_remote_sdp(SalOp *h, const char* sdp)
{
	if (sdp == NULL)
	{
		return -1;
	...
	h->base.remote_sdp = sdp;

	return 0;
}

char *sal_call_get_remote_sdp_str(SalOp *h) {
	return h->base.remote_sdp;
}
...
int sal_call(SalOp *h, const char *from, const char *to) {
	...
	err = eXosip_call_build_initial_invite(sal->osipCtx, &invite, to, from, route, "Phone call");
	if (err != 0) {
	...
	if (h->base.local_sdp != NULL && strlen(h->base.local_sdp)) {
		...
		snprintf(clen, sizeof(clen), "%i", sdplen);
		osip_message_set_body(invite, h->base.local_sdp, sdplen);
		osip_message_set_content_type(invite, "application/sdp");
		osip_message_set_content_length(invite, clen);

		h->sdp_offering = TRUE;
	}
	else if (h->base.local_media) {
		h->sdp_offering = TRUE;
		set_sdp_from_desc(invite, h->base.local_media);
	}
	else {
		h->sdp_offering = FALSE;
	}
	...
	err = eXosip_call_send_initial_invite(sal->osipCtx, invite);
...
int sal_call_notify_ringing(SalOp *h, bool_t early_media) {
	...
	if (early_media) {
		msg = NULL;
		eXosip_lock(sal->osipCtx);
		eXosip_call_build_answer(sal->osipCtx, h->tid, 183, &msg);
		if (msg) {
			sdp_process(h);
			if (h->sdp_answer) {
			...
			eXosip_call_send_answer(sal->osipCtx, h->tid, 183, msg);
		}
		eXosip_unlock(sal->osipCtx);
	}
	else {
		eXosip_lock(sal->osipCtx);
		eXosip_call_send_answer(sal->osipCtx, h->tid, 180, NULL);
		eXosip_unlock(sal->osipCtx);
	}
	return 0;
}
...
int sal_call_accept(SalOp * h) {
	...
	/* sends a 200 OK */
	int err = eXosip_call_build_answer(sal->osipCtx, h->tid, 200, &msg);
	if (err < 0 || msg == NULL) {
	...
	if (h->base.local_sdp != NULL && strlen(h->base.local_sdp) > 0)
	{
		...
		sdplen = strlen(h->base.local_sdp);
		snprintf(clen, sizeof(clen), "%i", sdplen);
		osip_message_set_body(msg, h->base.local_sdp, sdplen);
	...
	eXosip_call_send_answer(sal->osipCtx, h->tid, 200, msg);
	return 0;
}
...
int sal_call_decline(SalOp *h, SalReason reason, const char *redirect) {
	...
	if (reason == SalReasonBusy) {
		eXosip_lock(sal->osipCtx);
		eXosip_call_send_answer(sal->osipCtx, h->tid, 486, NULL);
		eXosip_unlock(sal->osipCtx);
	}
	else if (reason == SalReasonDeclined)
	{
		eXosip_lock(sal->osipCtx);
		eXosip_call_send_answer(sal->osipCtx, h->tid, 603, NULL);
		eXosip_unlock(sal->osipCtx);
	}
	else if (reason == SalReasonTemporarilyUnavailable) {
		eXosip_lock(sal->osipCtx);
		eXosip_call_send_answer(sal->osipCtx, h->tid, 480, NULL);
		eXosip_unlock(sal->osipCtx);
	}
	else if (reason == SalReasonDoNotDisturb) {
		eXosip_lock(sal->osipCtx);
		eXosip_call_send_answer(sal->osipCtx, h->tid, 600, NULL);
		eXosip_unlock(sal->osipCtx);
	}
	else if (reason == SalReasonMedia) {
		eXosip_lock(sal->osipCtx);
		eXosip_call_send_answer(sal->osipCtx, h->tid, 415, NULL);
		eXosip_unlock(sal->osipCtx);
	}
		...
		eXosip_lock(sal->osipCtx);
		eXosip_call_build_answer(sal->osipCtx, h->tid, code, &msg);
		osip_message_set_contact(msg, redirect);
		eXosip_call_send_answer(sal->osipCtx, h->tid, code, msg);
		eXosip_unlock(sal->osipCtx);
	}
...
static int send_notify_for_refer(SalOp *h, int did, const char *sipfrag) {
	osip_message_t *msg;
	Sal *sal = sal_op_get_sal(h);
	eXosip_lock(sal->osipCtx);
	eXosip_call_build_notify(sal->osipCtx, did, EXOSIP_SUBCRSTATE_ACTIVE, &msg);
	if (msg == NULL) {
		eXosip_unlock(sal->osipCtx);
	...
	eXosip_call_send_request(sal->osipCtx, did, msg);
	eXosip_unlock(sal->osipCtx);
	return 0;
}
...
int sal_call_notify_refer_state(SalOp *h, SalOp *newcall) {
	if (newcall == NULL) {
		/* in progress*/
		send_notify_for_refer(h, h->did, "SIP/2.0 100 Trying\r\n");
	}
		...
		if (newcall->did == -1) {
			...
			if (!newcall->terminated) {
				if (send_notify_for_refer(h, h->did, "SIP/2.0 200 Ok\r\n") == -1) {
...
int sal_ping(SalOp *op, const char *from, const char *to) {
	...
	eXosip_options_build_request(sal->osipCtx, &options, sal_op_get_to(op),
	...
	if (options) {
		...
		return eXosip_options_send_request(sal->osipCtx, options);
	}
	return -1;
}
...
int sal_call_refer(SalOp *h, const char *refer_to) {
	...
	int err = 0;
	eXosip_lock(sal->osipCtx);
	eXosip_call_build_refer(sal->osipCtx, h->did, refer_to, &msg);
	if (msg) err = eXosip_call_send_request(sal->osipCtx, h->did, msg);
	else err = -1;
	eXosip_unlock(sal->osipCtx);
	return err;
}
...
int sal_call_refer_with_replaces(SalOp *h, SalOp *other_call_h) {
	...
	int err = 0;
	eXosip_lock(sal->osipCtx);
	if (eXosip_call_get_referto(sal->osipCtx, other_call_h->did, referto, sizeof(referto) - 1) != 0) {
		ms_error("eXosip_call_get_referto() failed for did=%i", other_call_h->did);
		eXosip_unlock(sal->osipCtx);
		return -1;
	}
	eXosip_call_build_refer(sal->osipCtx, h->did, referto, &msg);
...
SalOp *sal_call_get_replaces(SalOp *h) {
	if (h != NULL && h->replaces != NULL) {
		...
		int cid;
		eXosip_lock(sal->osipCtx);
		cid = eXosip_call_find_by_replaces(sal->osipCtx, h->replaces);
		eXosip_unlock(sal->osipCtx);
		if (cid > 0) {
...
int sal_call_send_dtmf(SalOp *h, char dtmf) {
	...
	eXosip_lock(sal->osipCtx);
	eXosip_call_build_info(sal->osipCtx, h->did, &msg);
	if (msg) {
		...
		osip_message_set_content_length(msg, clen);
		eXosip_call_send_request(sal->osipCtx, h->did, msg);
	}
	eXosip_unlock(sal->osipCtx);
	return 0;
}

static void push_auth_to_exosip(SalOp *h, const SalAuthInfo *info) {
	Sal *sal = sal_op_get_sal(h);
	const char *userid;
	if (info->userid == NULL || info->userid[0] == '\0') userid = info->username;
	else userid = info->userid;
 ...
 * Just for symmetry ;-)
 */
static void pop_auth_from_exosip(SalOp *h) {
	Sal *sal = sal_op_get_sal(h);
	eXosip_clear_authentication_info(sal->osipCtx);
}
...
int sal_call_terminate(SalOp *h) {
	...
	if (h->auth_info) push_auth_to_exosip(h, h->auth_info);

	eXosip_lock(sal->osipCtx);
	err = eXosip_call_terminate(sal->osipCtx, h->cid, h->did);
	eXosip_unlock(sal->osipCtx);
...
static void set_network_origin(SalOp *op, osip_message_t *req) {
	...
	sal_op_set_network_origin(op, origin);
}
...
static void set_remote_contact(SalOp* op, osip_message_t *req) {
	if (op->base.remote_contact == NULL) {
		...
		if (h) {
			...
			sal_op_set_remote_contact(op, tmp);
			osip_free(tmp);
		}
	}
}
...
static void handle_reinvite(Sal *sal, eXosip_event_t *ev) {
	...
	if (op->base.remote_sdp != NULL)
	{
		ms_free(op->base.remote_sdp);
...
static int call_proceeding(Sal *sal, eXosip_event_t *ev) {
	...
	if (op == NULL || op->terminated == TRUE) {
		...
		eXosip_lock(sal->osipCtx);
		eXosip_call_terminate(sal->osipCtx, ev->cid, ev->did);
		eXosip_unlock(sal->osipCtx);
		return -1;
	}
	if (ev->did > 0)
...
static void call_accepted(Sal *sal, eXosip_event_t *ev) {
	...
	eXosip_call_build_ack(sal->osipCtx, ev->did, &msg);
	if (msg == NULL) {
		ms_warning("This call has been already terminated.");
		eXosip_lock(sal->osipCtx);
		eXosip_call_terminate(sal->osipCtx, ev->cid, ev->did);
	...
	eXosip_call_send_ack(sal->osipCtx, ev->did, msg);
...
static bool_t process_authentication(Sal *sal, eXosip_event_t *ev) {
	...
	osip_cseq_t *cseq = osip_message_get_cseq(ev->response);
	if (strcasecmp(osip_cseq_get_method(cseq), "OPTIONS") == 0)
	{
		...
		*/
		ms_message("process_authentication(): ignored for ping request.");
		return TRUE;
	}
...
static void process_media_control_xml(Sal *sal, eXosip_event_t *ev) {
	...
	if (body && body->body != NULL &&
		...
		if (sal->callbacks.vfu_request) {
			...
			eXosip_call_build_answer(sal->osipCtx, ev->tid, 200, &ans);
			if (ans)
				eXosip_call_send_answer(sal->osipCtx, ev->tid, 200, ans);
			return;
		}
	}
	/*in all other cases we must say it is not implemented.*/
	{
		osip_message_t *ans = NULL;
		eXosip_lock(sal->osipCtx);
		eXosip_call_build_answer(sal->osipCtx, ev->tid, 501, &ans);
		if (ans)
			eXosip_call_send_answer(sal->osipCtx, ev->tid, 501, ans);
		eXosip_unlock(sal->osipCtx);
	}
}
...
static void process_refer(Sal *sal, SalOp *op, eXosip_event_t *ev) {
	...
	if (h) {
		...
		eXosip_lock(sal->osipCtx);
		eXosip_call_build_answer(sal->osipCtx, ev->tid, 202, &ans);
		if (ans)
			eXosip_call_send_answer(sal->osipCtx, ev->tid, 202, ans);
		eXosip_unlock(sal->osipCtx);
	}
	else
	{
...
static void inc_update(Sal *sal, eXosip_event_t *ev) {
	...
	eXosip_lock(sal->osipCtx);
	eXosip_message_build_answer(sal->osipCtx, ev->tid, 200, &msg);
	if (msg != NULL)
		eXosip_message_send_answer(sal->osipCtx, ev->tid, 200, msg);
	eXosip_unlock(sal->osipCtx);
}
...
static void other_request(Sal *sal, eXosip_event_t *ev) {
	...
	if (strcmp(ev->request->sip_method, "MESSAGE") == 0) {
		text_received(sal, ev);
		eXosip_lock(sal->osipCtx);
		eXosip_message_send_answer(sal->osipCtx, ev->tid, 200, NULL);
		eXosip_unlock(sal->osipCtx);
	}
	else if (strcmp(ev->request->sip_method, "OPTIONS") == 0) {
		osip_message_t *options = NULL;
		eXosip_lock(sal->osipCtx);
		eXosip_options_build_answer(sal->osipCtx, ev->tid, 200, &options);
		fill_options_answer(options);
		eXosip_options_send_answer(sal->osipCtx, ev->tid, 200, options);
		eXosip_unlock(sal->osipCtx);
	}
		...
		eXosip_lock(sal->osipCtx);
		eXosip_message_send_answer(sal->osipCtx, ev->tid, 501, NULL);
		eXosip_unlock(sal->osipCtx);
	}
}
...
static void other_request_reply(Sal *sal, eXosip_event_t *ev) {
	...
	if (ev->response) {
		ms_message("Processing response status [%i] for method [%s]", ev->response->status_code, osip_message_get_method(ev->request));
		update_contact_from_response(op, ev->response);
	...
	if (ev->request && strcmp(osip_message_get_method(ev->request), "MESSAGE") == 0) {
		/*out of call message acknowledgement*/
	...
	if (ev->response) {
		if (ev->request && strcmp(osip_message_get_method(ev->request), "NOTIFY") == 0) {
			if (op->sipfrag_pending) {
				send_notify_for_refer(op, op->did, op->sipfrag_pending);
...
static bool_t process_event(Sal *sal, eXosip_event_t *ev) {
	ms_message("linphone process event get a message type %d\n", ev->type);
	switch (ev->type) {
	...
	case EXOSIP_REGISTRATION_SUCCESS:
		ms_message("REGISTRATION_SUCCESS\n");
	...
	case EXOSIP_NOTIFICATION_REQUESTFAILURE:
		if (ev->response) {
			...
			case 412: {
				eXosip_automatic_action(sal->osipCtx);
				return TRUE;
			}
			}
		}
...
int sal_iterate(Sal *sal) {
	eXosip_event_t *ev;
	while ((ev = eXosip_event_wait(sal->osipCtx, 0, 50)) != NULL) {
...
int sal_register(SalOp *h, const char *proxy, const char *from, int expires) {
	...
	if (h->rid == -1) {
		...
		eXosip_lock(sal->osipCtx);
		h->rid = eXosip_register_build_initial_register(sal->osipCtx, from, domain, NULL, expires, &msg);
		if (msg) {
		...
		eXosip_lock(sal->osipCtx);
		eXosip_register_build_register(sal->osipCtx, h->rid, expires, &msg);
		sal_message_add_route(msg, proxy);
	}
	if (msg) {
		eXosip_register_send_register(sal->osipCtx, h->rid, msg);
	}
	eXosip_unlock(sal->osipCtx);
	h->expires = expires;
	return (msg != NULL) ? 0 : -1;
...
int sal_register_refresh(SalOp *op, int expires) {
	...
	eXosip_lock(sal->osipCtx);
	eXosip_register_build_register(sal->osipCtx, op->rid, expires, &msg);
	if (msg != NULL) {
		if (contact) register_set_contact(msg, contact);

		sal_message_add_route(msg, sal_op_get_route(op));
		eXosip_register_send_register(sal->osipCtx, op->rid, msg);
	}
...
int sal_unregister(SalOp *h) {
	...
	Sal *sal = sal_op_get_sal(h);
	eXosip_lock(sal->osipCtx);
	eXosip_register_build_register(sal->osipCtx, h->rid, 0, &msg);
	if (msg) eXosip_register_send_register(sal->osipCtx, h->rid, msg);
...
void sal_set_keepalive_period(Sal *ctx, unsigned int value) {
	...
	eXosip_set_option(ctx->osipCtx, EXOSIP_OPT_UDP_KEEP_ALIVE, &ctx->keepalive_period);
}
...
int sal_call_update(SalOp *h, const char *subject) {
	...
	eXosip_lock(sal->osipCtx);
	if (eXosip_call_build_request(sal->osipCtx, h->did, "INVITE", &reinvite) != 0 || reinvite == NULL) {
		eXosip_unlock(sal->osipCtx);
		return -1;
	}
	eXosip_unlock(sal->osipCtx);
	...
	if (h->base.local_sdp != NULL && strlen(h->base.local_sdp)) {
		...
		sdplen = strlen(h->base.local_sdp);
		snprintf(clen, sizeof(clen), "%i", sdplen);
		osip_message_set_body(reinvite, h->base.local_sdp, sdplen);
	...
	eXosip_lock(sal->osipCtx);
	err = eXosip_call_send_request(sal->osipCtx, h->did, reinvite);
	eXosip_unlock(sal->osipCtx);
	return err;
}
