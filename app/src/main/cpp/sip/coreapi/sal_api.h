...
#ifndef __LINPHONE_SAL_H__
#define __LINPHONE_SAL_H__
...
int sal_media_decription_new_from_str(const char *sdp, SalMediaDescription *desc);
...
typedef struct SalOpBase{
	...
	char *local_sdp;	/* sdp from webrtc */
	char *remote_sdp; /* sdp from remote */
	void *user_pointer;
	char* call_id;
	char *remote_contact;
...
/*internal API */
void sal_op_init(SalOp *b, Sal *sal);
void sal_op_set_network_origin(SalOp *op, const char *origin /*a sip uri*/);
void sal_op_set_remote_contact(SalOp *op, const char *ct);
void sal_op_free(SalOp *b);

#endif // __LINPHONE_SAL_H__
