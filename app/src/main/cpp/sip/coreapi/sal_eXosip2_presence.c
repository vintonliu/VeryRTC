...
int sal_message_send(SalOp *op, const char *from, const char *to, const char* content_type, const char *msg){
	...
	if(op->cid == -1)
	{
		...
		eXosip_message_build_request(sal->osipCtx, &sip,"MESSAGE",sal_op_get_to(op),
		...
		if (sip!=NULL){
			...
			eXosip_message_send_request(sal->osipCtx, sip);
		}else{
...
int sal_subscribe_accept(SalOp *op){
	...
	Sal *sal = sal_op_get_sal(op);
	eXosip_lock(sal->osipCtx);
	eXosip_insubscription_build_answer(sal->osipCtx, op->tid,202,&msg);
	if (msg==NULL){
	...
	eXosip_insubscription_send_answer(sal->osipCtx, op->tid,202,msg);
	eXosip_unlock(sal->osipCtx);
	return 0;
}
...
int sal_subscribe_decline(SalOp *op){
	Sal *sal = sal_op_get_sal(op);
	eXosip_lock(sal->osipCtx);
	eXosip_insubscription_send_answer(sal->osipCtx, op->tid,401,NULL);
	eXosip_unlock(sal->osipCtx);
	return 0;
}
...
int sal_notify_presence(SalOp *op, SalPresenceStatus status, const char *status_message){
	...
	eXosip_lock(sal->osipCtx);
	eXosip_insubscription_build_notify(sal->osipCtx, op->did, EXOSIP_SUBCRSTATE_ACTIVE, DEACTIVATED, &msg);
	if (msg!=NULL){
		...
		eXosip_insubscription_send_request(sal->osipCtx, op->did,msg);
...
int sal_notify_close(SalOp *op){
	...
	Sal *sal = sal_op_get_sal(op);
	eXosip_lock(sal->osipCtx);
	eXosip_insubscription_build_notify(sal->osipCtx, op->did,EXOSIP_SUBCRSTATE_TERMINATED,DEACTIVATED,&msg);
	if (msg!=NULL){
		const char *identity=sal_op_get_contact(op);
		if (identity==NULL) identity=sal_op_get_to(op);
		osip_message_set_contact(msg,identity);
...
int sal_publish(SalOp *op, const char *from, const char *to, SalPresenceStatus presence_mode){
	...
	i = eXosip_build_publish(sal->osipCtx, &pub,to, from, NULL, "presence", "600",
	...
	eXosip_lock(sal->osipCtx);
	i = eXosip_publish(sal->osipCtx, pub, to); /* should update the sip-if-match parameter
...
void sal_exosip_subscription_recv(Sal *sal, eXosip_event_t *ev){	
	...
	if (op){
		...
		if (h && h->hvalue && atoi(h->hvalue)==0){
			ms_warning("This subscribe is not a new one but terminates an old one.");
			ev->did=op->did;
			ev->nid=op->nid;
			...
			eXosip_lock(sal->osipCtx);
			eXosip_insubscription_build_answer(sal->osipCtx, ev->tid,202,&msg);
			eXosip_insubscription_send_answer(sal->osipCtx, ev->tid,202,msg);
			eXosip_unlock(sal->osipCtx);
		}
